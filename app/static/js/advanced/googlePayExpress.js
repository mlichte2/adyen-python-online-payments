  ...
  try {
    ...
    const googlePayExpressConfig = {
      ...
      transactionInfo: {
        displayItems: [
          {
            label: "Subtotal",
            type: "SUBTOTAL",
            price: paymentMethodsData.amount.value,
          },
          {
            label: "Tax",
            type: "TAX",
        ...
        currencyCode: paymentMethodsData.amount.currency,
        totalPriceStatus: "FINAL",
        totalPrice: paymentMethodsData.amount.value + 1,
    ...
    console.log(googlePayExpressConfig);

    const checkout = await AdyenCheckout(configuration);
    const googlePay = new GooglePay(checkout, googlePayExpressConfig).mount(
      "#component-container"
    );
  } catch (error) {
    console.error(error);
