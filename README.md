...
4.  **Run the Setup Script**
    ...
    ./setup.sh
    ```
    > **Note:** This script uses [**`uv sync`**](https://docs.astral.sh/uv/reference/cli/#uv-sync) to create a virtual environment (`.venv`) and install all the required Python packages from `requirements.txt`.
...
1.  **Start the Server**
    ...
    ./start.sh
    ```
    > **Note:** This script uses [**`uv run`**](https://docs.astral.sh/uv/reference/cli/#uv-run) to launch the app inside the `.venv` created by `setup.sh`, without needing to activate the virtual environment manually.
...
## ⬇️ Getting Updates (New to Git?)

This project gets new features and fixes over time (new components, bug fixes, the configuration explorer, etc.). If you're not familiar with `git`, here's the simplest way to pull down the latest changes.

> **Heads up:** If you've made your own edits to files in this repo (e.g. changed `card.js` for a test), a plain `git pull` can fail or overwrite your work. The steps below temporarily set your changes aside so they don't get lost, then bring the latest code down.

1.  **Open a terminal in the project folder** (the same one where you ran `./setup.sh`).

2.  **Save your local changes for safekeeping.** This "stashes" any edits you've made so your working directory is clean:

    ```bash
    git stash
    ```

    > If you haven't changed anything, this command will just say "No local changes to save" — that's fine, continue to the next step.

3.  **Download and apply the latest code:**

    ```bash
    git pull
    ```

4.  **Restore your local changes on top of the new code:**

    ```bash
    git stash pop
    ```

    > If your edits touched the same lines as an incoming update, Git will flag a **merge conflict** in that file (you'll see `<<<<<<<` markers in it). Open the file, decide which version of that section to keep, remove the markers, and save. If this looks intimidating, reach out to whoever shared this project with you for help.

5.  **Re-run the setup script** in case any dependencies changed:

    ```bash
    ./setup.sh
    ```

6.  **Check your `.env` file.** New releases occasionally add or rename environment variables (see `example.env` for the current full list). Compare your `.env` against `example.env` and add anything you're missing.

> **Tip:** If you never intend to keep custom edits and just want the latest version every time, you can skip the `git stash` step. Just know that `git pull` will refuse to run if it would overwrite changed files, so you'd need to either commit or discard those changes first.
